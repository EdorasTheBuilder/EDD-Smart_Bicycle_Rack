...
#-----Vars-----
stall_list = [
    {'name':1 ,'status': False, 'bar_out': 27, 'bar_in': 22, 'cable_out': 5, 'cable_in': 6, }
]
...
while True:
    for i in stall_list:
        if i.get('status') == True:
            if pi.read(i.get('bar_in')) == 0:
                print('Stall' + i.get('name'))
                print('bar signal lost')
            if pi.read(i.get('cable_in')) == 0:
                print('Stall' + i.get('name'))
                print('cable signal lost')
        time.sleep(1)
