...
def unlock(stall): #spins a servo to unlock it 
    ...
    time.sleep(1)
    pi.set_servo_pulsewidth(pin, unlock_pos)
    time.sleep(1)
...
def rack_read(stall):
    ...
    if bar_signal == 0 and cable_signal == 0:
        print('\n Current lost from locking bar and cable at stall ' + str(stall.get('name')))
        secure = False
    elif bar_signal == 0: 
        print('\n Current lost from locking bar at stall' + str(stall.get('name')))
        secure = False
    elif cable_signal == 0: 
...
def main(users, stall_list, email, user_pin):
    use = input("""Welcome! 
...
Please type "L" for locking or "U" for unlocking. """)
    ...
    if use == "L" or use == 'l':
        ...
        for stall in stall_list:
            if stall.get('assigned') == False:
                ...
                "1. Slide your bicycle into the cage.", 
                "2. Slide the metal bar through your frame. DO NOT push it all the way though yet", 
                "3. Wrap the cable through your wheel. ", 
                "4. Now push the bar through the end. ", 
            
                
                ]
                
                lst_print(locking_instructions)
    ...
    ######### unlocking            
        ...
        unlocking_instructions = [
            ...
            '\n',
            'Welcome back!',
            'Once you have unlocked your bicycle, please slide the bar back in place.'


        ]

        lst_print(unlocking_instructions)
        ...
        if verify == True:
            ...
            for stall in stall_list:
                ...
                if stall.get('user') == email:
                    unlock(stall)
                    print('You may now proceed to stall ' + stall.get('name') + ' and pull on the bar to remove your bicycle')
...
def take_input():
    while thread_running == True: 
        main(users, stall_list, email, user_pin)
        time.sleep(1)
        os.system('clear') 
